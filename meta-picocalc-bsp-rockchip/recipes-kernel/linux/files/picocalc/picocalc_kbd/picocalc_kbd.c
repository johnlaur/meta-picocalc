...
static int picocalc_kbd_reboot_notifier(struct notifier_block *nb,
                              unsigned long action, void *data)
{
    if (action == SYS_POWER_OFF || action  == SYS_HALT) {
        ...
        // Register REG_ID_OFF
        kbd_write_i2c_u8(g_ctx->i2c_client, 0x0e, KBD_POWEROFF_SEC);
    }
    return NOTIFY_DONE;
}
...
#endif
{
    ...
    */
    ...
    g_ctx->reboot_nb.notifier_call = picocalc_kbd_reboot_notifier;
...
{
    unregister_reboot_notifier(&g_ctx->reboot_nb);
